...
static unsigned int chartorune(char** buf)
{
	int c, c1, c2;
	long l;
	char* str = *buf;
...
static int runetochar(char *str, unsigned int rune)
{
	long c;
...
static unsigned int* bsearch(unsigned int c, unsigned int *t, 
	int n, int ne) {
...
static unsigned int runetolower(unsigned int c)
{
	...
	p = bsearch(c, tolower2, nelem(tolower2) / 3, 3);
	if (p && c >= p[0] && c <= p[1]) {
		return c + p[2] - 500;
	}

	p = bsearch(c, tolower1, nelem(tolower1) / 2, 2);
	if (p && c == p[0]) {
		return c + p[1] - 500;
	}
...
unsigned int Utf8::runetoupper(unsigned int c)
{
	unsigned* p;

	p = bsearch(c, toupper2, nelem(toupper2) / 3, 3);
	if (p && c >= p[0] && c <= p[1]) {
		return c + p[2] - 500;
	}
...
int Utf8::toupper(const string& str, string& result) {
	int i = 0;
...
int Utf8::tolower(const string& str, string& result) {
	int i = 0;
	const char* s = str.c_str();
	char buf[10];
